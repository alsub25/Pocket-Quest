  ...
  switch (merchantId) {
    ...
    case 'arcanist':
      return [
        'staffOak',
        'robeApprentice',
        ...(state.player.resourceKey === 'mana' && resPotionKey ? [resPotionKey] : [])
      ];

    case 'alchemist':
      // All basic potion types that exist in ITEM_DEFS
      return ['potionSmall', 'potionMana', 'potionFury', 'potionBlood'];

    case 'provisioner':
      // A generalist mix; always has small heal + your class resource pot + some light armor.
      return ['potionSmall', ...(resPotionKey ? [resPotionKey] : []), 'armorLeather'];
...
function getTravelingMerchantStock(state) {
  ...
  if (state.player.resourceKey === 'mana') {
    base.push('potionMana', 'staffOak');
  } else if (state.player.resourceKey === 'fury') {
    base.push('potionFury', 'swordIron');
  } else if (state.player.resourceKey === 'blood') {
...
function ensureMerchantStock(state, context, merchantId, stockKeys, cloneItemDef) {
...
function buildItemRow({
  ...
  const finalPrice = getMerchantPrice(basePrice, state, context);
  ...
  btnBuy.addEventListener('click', () => {
    ...
    handleEconomyAfterPurchase(state, finalPrice, context);
    updateHUD();
    saveGame();
    refreshUI();
  });
...
function openVillageMerchantHub({
  ...
  openModal('Village Merchants', body => {
    ...
    econLine.textContent =
      `Village economy: ${tier.name} – prices are ${tier.priceDescriptor}.`;
    ...
    VILLAGE_MERCHANTS.forEach(merchant => {
      ...
      const left = document.createElement('div');
      left.innerHTML =
        `<span class="item-name">${merchant.icon} ${runtimeName}</span>`;
...
function openTravelingMerchantShop({
  ...
  openModal(`${travelerName}, Traveling Merchant`, body => {
    ...
    econLine.className = 'modal-subtitle';
    econLine.textContent =
      `Village economy: ${tier.name} – traveling merchants adjust their prices accordingly.`;
    body.appendChild(econLine);
